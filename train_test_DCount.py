...
if __name__ == '__main__':
    ...
    parser.add_argument('--stats_dir', default='./stats_0931')
    ...
    if args.resume_checkpoint:
        ...
        model = load_model(args.config, args.resume_checkpoint, device=device, DCount=True).to(device)
    else:
        model = load_model(args.config, args.pretrained_checkpoint, device=device, DCount=True).to(device)

    # 冻结backbone和bert
    model = freeze_encoders(model)
    # 损失函数
    criterion = SetCriterion(use_contrast_asd=True, cost_rep=0)
    # 优化器
    ...
    # 加载最佳模型，并设置为测试模式
    test_model = load_model(args.config, model_name, device=device, DCount=True).to(device)
