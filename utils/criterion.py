...
class SetCriterion(nn.Module):
    ...
    def loss_label(self, outputs, targets, indices, num_points, caption_sizes, **kwargs):
        ...
        idx = self._get_src_permutation_idx(
            indices)  # tuple ((b#1,b#1,b#1,b#1..., b#2,b#2,b#2...), (pred_idx, pred_idx, pred_idx...)) # 0, 1 size: sum(nt)
        target_classes_o = torch.cat([t["labels"][J] for t, (_, J) in zip(targets,
                                                                          indices)])  # (nt, 256) re-ordered in pred_idx order but using target labels
